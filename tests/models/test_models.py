...
def test_system_payload_creation():
    """测试系统消息载荷创建"""
    payload = SystemPayload(type="ctrl", content={"op": "join", "env_id": "room_1"})
...
def test_message_payload_creation():
    """测试业务消息载荷创建"""
    payload = MessagePayload(type="action", content={"name": "move", "x": 10, "y": 5})
...
def test_broadcast_payload_creation():
    ...
    payload = BroadcastPayload(
        type="event", content={"name": "night_fall", "vision_modifier": 0.5}
    )
...
def test_envelope_with_message_payload():
    ...
    envelope = Envelope(
        ...
        payload=MessagePayload(
            type="action", content={"name": "move", "x": 10, "y": 5}
        ),
    )
...
def test_envelope_with_broadcast_payload():
    ...
    envelope = Envelope(
        ...
        sender="env_main",
        recipient="@all",
        payload=BroadcastPayload(type="event", content={"name": "time_tick"}),
    )
...
def test_envelope_dict_conversion():
    ...
    envelope = Envelope(
        ...
        sender="hub",
        recipient="agent_01",
        payload=SystemPayload(type="notify", content={"message": "Connected"}),
    )
...
def test_envelope_discriminator():
    ...
    envelope_dict = {
        ...
        "sender": "hub",
        "recipient": "agent_01",
        "payload": {"type": "notify", "content": {"message": "test"}},
    }
