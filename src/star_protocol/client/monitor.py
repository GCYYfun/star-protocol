...
class InMemoryStorage:
    ...
    async def get_messages(
        self, client_id: str, data_type: Optional[str] = None, limit: int = 100
    ) -> List[Dict]:
        """获取消息历史"""
        if data_type:
...
class MonitorClient(BaseClient):
    ...
    def __init__(
        self, monitor_id: str, storage: Optional[InMemoryStorage] = None, **kwargs
    ):
        """
        初始化 Monitor 客户端
        ...
        """
        # 先调用父类初始化
        super().__init__(client_id=monitor_id, role="monitor", **kwargs)
        ...
        await self.send(
            Envelope(
                ...
                payload=MonitorPayload(
                    ...
                    content={"op": "subscribe", "target_client_id": client_id},
                ),
            )
        )
    ...
    async def unsubscribe_from_client(self, client_id: str) -> None:
        ...
        await self.send(
            Envelope(
                ...
                payload=MonitorPayload(
                    type=MonitorType.CTRL,
                    content={"op": "unsubscribe", "target_client_id": client_id},
                ),
            )
        )
    ...
    async def on_monitor_data(
        self, client_id: str, data_type: str, data: Dict[str, Any]
    ) -> None:
        """
        监控数据回调（子类可重写）
