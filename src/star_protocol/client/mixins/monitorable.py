...
class MonitorableMixin:
        ...
        await self.send(
            Envelope(
                ...
                payload=MonitorPayload(
                    type=MonitorType.CTRL, content={"op": "enable", "level": level.name}
                ),
            )
        )
    ...
    async def disable_monitoring(self) -> None:
        ...
        await self.send(
            Envelope(
                ...
                recipient="hub",
                payload=MonitorPayload(
                    type=MonitorType.CTRL, content={"op": "disable"}
                ),
            )
        )
    ...
    async def _send_monitor_data(self, data_name: str, data: Dict[str, Any]) -> None:
        """
        发送监控数据到 Hub
    ...
    async def _on_state_change(self, old_state: str, new_state: str, **kwargs) -> None:
        ...
        await self._send_monitor_data(
            "state_change", {"old_state": old_state, "new_state": new_state, **kwargs}
        )
    ...
    async def _on_message_sent(self, envelope: Envelope) -> None:
        ...
        await self._send_monitor_data(
            "message_sent",
            {
                ...
                if hasattr(envelope.payload, "type")
                else None,
    ...
    async def _handle_monitor(self, envelope: Envelope) -> None:
        ...
        if payload.type == MonitorType.NOTIFY:
            ...
            if event == "monitoring_enabled":
                ...
                self.logger.info(
                    f"Monitoring enabled confirmed by Hub (level: {level})"
                )
            ...
            self.logger.warning(
                f"Unexpected MonitorType.DATA received by {self.client_id}"
            )

        elif payload.type == MonitorType.CTRL:
            # MonitorableMixin 不应该接收 CTRL 类型（只发送不接收）
            self.logger.warning(
                f"Unexpected MonitorType.CTRL received by {self.client_id}"
            )
